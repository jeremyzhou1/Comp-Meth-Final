...
public class Final {
	public static void main(String[] args) throws IOException{
		...
		for (int i = 0; i < 3; i++){
			...
			double[][] result1 = mult(data.get(i), 2);
			
			timeTaken = System.nanoTime() - start;
			System.out.println("Matrix size: " + matSizes[i] + ", Time Taken: " + timeTaken); //output
			printMatrix(result1);
			System.out.println();
		}
		...
		for (int i = 0; i < 3; i++){
			...
			double[][] result1 = diagMult(data.get(i), 2);
	...
	public static double[][] mult(double[][] mat1, int power) {
		int n = mat1.length;
		double value = 0;
		...
		for (int i = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				value = 0;
				a = mat1[i];
				for (int k = 0; k < n; k++)	b[k] = mat1[k][j];
	...
	public static double[][] diagMult(double[][] mat, int power) {
		int n = mat.length;
		double[][] result = new double[n][n];
